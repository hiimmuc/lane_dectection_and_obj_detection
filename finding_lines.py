...
def draw_lane(img, left_line, right_line, lane_color=[(0, 0, 255),(255,0,0)], road_color=(0, 255, 0)):
    ...
    cv2.fillPoly(window_img, np.int_([left_pts]), lane_color[0])
    cv2.fillPoly(window_img, np.int_([right_pts]), lane_color[1])
...
def print_road_map(image, left_line, right_line):
    ...
    # Draw the lane onto the warped blank image
    cv2.fillPoly(window_img, np.int_([left_pts]), (0, 0, 170))
    cv2.fillPoly(window_img, np.int_([right_pts]), (170, 0, 0))
    ...
    # Draw the lane onto the warped blank image
    cv2.fillPoly(window_img, np.int_([pts]), (0, 150, 0))
